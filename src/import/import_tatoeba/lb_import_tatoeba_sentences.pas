...
function UpdateTatoebaSentences(cn: TSqlConnection) : integer;
var q, u : TSqlQuery;
 sentenceId, sourceId, soundId : integer;
 sentence : string;
begin
  result := 0;
  ...
  try
    ...
    begin
      ...
      sourceId := q.FieldByName('source_id').AsInteger;
      ...
      soundId := InsertSound(cn, sourceTypeIdTatoeba, format('%d.mp3', [sourceId]));
...
procedure InsertIntoLangBathSentenceTables(cn: TSqlConnection; target : byte;
    maxLevel : integer;
    back : boolean = false);
const
    baseQuery
...
begin
  sql := format('insert into sentences(sentence,source_id,source_type_id)'
    + ' select sentence,tatoeba_id,%d'
  ...
  rows := ExecuteSql(cn, baseQuery, [distance, join, tablePrefix, SourceTypeIdTatoeba, target]);
